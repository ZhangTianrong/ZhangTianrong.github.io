...
在此，也给出我目前在使用的配置，在默认文件上仅针对powershell进行了修改，以后会加入对WSL的配置。我们需要先行下载所需的字体文件，[该字体](https://github.com/sharpglasses/Consolas-with-Yahei/tree/master/ttf)在糅合Consolas和雅黑之外还添加了Powerline字符，这在接下来`oh-my-posh`模块的显示中有着至关重要的作用。
